...
class EventRoute {
   public function store() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if (isset($_POST['volunteerBody'])) {
            $bodySize = strlen($_POST['volunteerBody']);
            ...
            if ($bodySize > $maxBodySize) {
                ...
                echo json_encode([
                    "success" => false,
                    "message" => "Your event content is too large ({$kbSize} KB). Please remove or reduce embedded images. Maximum allowed size is {$maxKB} KB."
                ]);
                exit;
            }
        }
        ...
        if ($result['success']) {
            ...
            // ✅ Return actual database or validation error
            echo json_encode([
                "success" => false,
                "message" => $result['message'] ?? "Unknown error occurred while creating event."
            ]);
        }
    }
}
