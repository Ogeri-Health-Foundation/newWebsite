...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        try {
            ...
            // 2. Handle image uploads and deletions
            $hasNewUploads = false;
            ...
            if (!empty($deletedImages)) {
                foreach ($deletedImages as $deletedImage) {
                    ...
                    $deleteQuery = "DELETE FROM event_galleries WHERE event_id = :event_id AND img_path = :img_path";
                    $stmt = $dbh->prepare($deleteQuery);
                    $stmt->execute(['event_id' => $eventId, 'img_path' => $deletedImage]);

                    // Remove physical file
                    $filePath = "../../uploads/events/" . $deletedImage;
                    if (file_exists($filePath)) {
                        unlink($filePath);
                    }
                }
            ...
            // Handle new uploads
            if ($hasNewUploads) {
                // If user uploads new images, replace ALL existing images
                ...
                // Delete all existing images from database and filesystem
                $deleteAllQuery = "DELETE FROM event_galleries WHERE event_id = :event_id";
                $stmt = $dbh->prepare($deleteAllQuery);
                $stmt->execute(['event_id' => $eventId]);
                ...
                for ($i = 1; $i <= 6; $i++) {
                    ...
                    if (isset($_FILES[$fieldName]) && $_FILES[$fieldName]['error'] === UPLOAD_ERR_OK) {
                        ...
                        if (move_uploaded_file($_FILES[$fieldName]['tmp_name'], $targetPath)) {
                            $galleryId = 'gallery_' . bin2hex(random_bytes(6));
                            $galleryId = substr($galleryId, 0, 20);
            ...
            // If no new uploads and no deletions, existing images remain unchanged

            // Commit transaction
            $dbh->commit();

            // Get updated count of images
            $countQuery = "SELECT COUNT(*) as image_count FROM event_galleries WHERE event_id = :event_id";
            $stmt = $dbh->prepare($countQuery);
            $stmt->execute(['event_id' => $eventId]);
            $imageCount = $stmt->fetch(PDO::FETCH_ASSOC)['image_count'];

            echo json_encode([
                'success' => true,
                'message' => "Event details updated successfully. {$imageCount} images stored.",
